...
async def fill_nlf(req: FillRequest):
    try:
        ...
        for mapping in req.mappings:
            ...
            # Prefer the exact row from Step 3 mapping when provided
            if mapping.row:
                col_idx = col_letter_to_index(mapping.col) if mapping.col else 3
                ws.cell(row=mapping.row, column=col_idx).value = mapping.mapped_value
                filled += 1
                continue
